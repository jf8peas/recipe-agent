...
- [ ] T050 [US1] Implement `POST /api/recipe/:sid/step/commit` in `app/api/recipe/[sid]/step/commit/route.ts`: retries only the checkpoint write, never re-invokes the node (spec FR-080–FR-082)
- [ ] T051 [P] [US1] Write `tests/contract/step-commit.test.ts`, including: a commit retry does NOT re-invoke the node and does NOT write an extra `usage_events` row (spec FR-082/SC-026)
...
- [ ] T069 [P] [US2] Write `tests/contract/history.test.ts` including a session with 2 branches — asserts entries from both threads appear, each with the right `threadId`
- [ ] T070 [P] [US2] Implement `GET /api/recipe/:sid/state` in `app/api/recipe/[sid]/state/route.ts`, taking `branchId` + `checkpointId` query params (spec FR-021)
- [ ] T071 [P] [US2] Write `tests/contract/state.test.ts` including the unknown-checkpoint 400 and a `branchId` that belongs to a different session (404)
