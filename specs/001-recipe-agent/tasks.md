...
- [X] T054 [P] [US1] Implement `hooks/useSession.ts`: tracks `sessionId` + the active `branchId` + selected `checkpointId`; fetches `/state` and `/history`; `start` / `step` / `step` `mode:"retry"` / `step/commit` / `fork` mutations, all sending `branchId` where the contract requires it
- [X] T055 [P] [US1] Implement `components/RunningStage.tsx`: indeterminate spinner, elapsed-time counter, current stage name, Cancel button (spec FR-009a)
- [X] T056 [US1] Wire Cancel through `AbortController` in `useSession` (depends on T054, T055; spec FR-072–FR-075)
- [X] T057 [P] [US1] Implement `hooks/useAutoRun.ts`: client loop of single `/step` calls; reads the pause-between-stages value (T032); stops at finalize, ingredient-error, stage-failure, save-failure, cancel, any limit, or Pause (spec FR-035, FR-038); unit-test in `tests/unit/use-auto-run.test.ts` that a Pause during a run halts within one stage (spec SC-012)
- [X] T058 [US1] Implement Step/Play and Pause controls in `components/ActionToolbar.tsx` (the pause-between-stages toggle lives in `AppHeader`, T032) (depends on T054, T057)
- [X] T059 [P] [US1] Implement `components/UnsavedResultBanner.tsx` + "retry save" action calling `step/commit` (spec FR-081)
- [X] T060 [P] [US1] Implement `components/StageFailureBanner.tsx`: when the current saved state's kind is `stage-failure`, show `failureReason` and a **Retry** action (calls `useSession` `step` with `mode:"retry"`), plus a hint that Edit & Fork also works; disabled when the session is capped (FR-077) or an advance is in flight; wire into the session view (spec FR-051–FR-052; the timeline's visual distinctness, FR-053, is delivered by `BranchTimeline` in T072)
...
- [X] T062 [P] [US1] Implement a minimal read-only `components/fields/IngredientsEditor.tsx` showing per-item usable/reason (spec FR-043)
...
- [X] T072 [US2] Implement `components/BranchTimeline.tsx`: render the **unified cross-branch tree** from `lib/tree.ts` (T029), one row per saved state, kind badge (normal / in-progress / ingredient-error / stage-failure) as text + ARIA, not color alone (spec FR-017–FR-019, FR-053, FR-085)
